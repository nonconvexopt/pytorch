...
class _Symmetric(_Square):
    ...
    def check(self, value):
        assert super().check(value).all(), "Cannot check symmetricity if the input is not a square matrix."
...
class _PositiveDefinite(_Symmetric):
    ...
    def check(self, value):
        assert super().check(value).all(), "Cannot check positive definiteness if the input is not a symmetric matrix."
